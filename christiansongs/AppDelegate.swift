...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        //this part
        if (!NSFileManager.defaultManager().fileExistsAtPath(urljs.path!)) {
            let defaultStorePath = NSBundle.mainBundle().pathForResource("script", ofType:"js")
            
            NSFileManager.defaultManager().copyItemAtPath(defaultStorePath!, toPath:urljs.path!, error: nil)
        }

        
        let urlcss = self.applicationDocumentsDirectory.URLByAppendingPathComponent("style.css")
        
        //this part
        if (!NSFileManager.defaultManager().fileExistsAtPath(urlcss.path!)) {
            let defaultStorePath = NSBundle.mainBundle().pathForResource("style", ofType:"css")
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("christiansongs", withExtension: "momd")!
        return NSManagedObjectModel(contentsOfURL: modelURL)!
    }()
    lazy var managedObjectModelUserData: NSManagedObjectModel = {
        // The managed object model for the application. This property is not optional. It is a fatal error for the application not to be able to find and load its model.
        let modelURL = NSBundle.mainBundle().URLForResource("songbookmarks", withExtension: "momd")!
        return NSManagedObjectModel(contentsOfURL: modelURL)!
    }()
    /*lazy var managedObjectModelAllSongs: NSManagedObjectModel = {
        // The managed object model for the application. This property is not optional. It is a fatal error for the application not to be able to find and load its model.
        let modelURL = NSBundle.mainBundle().URLForResource("christiansongs", withExtension: "momd")!
        return NSManagedObjectModel(contentsOfURL: modelURL)!
    }()
    */
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        //let url = NSBundle.mainBundle().URLForResource("songs", withExtension: "sqlite")!
        ...
        //let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("christiansongs.sqlite")
    ...
    lazy var persistentStoreCoordinatorUserData: NSPersistentStoreCoordinator? = {
        // The persistent store coordinator for the application. This implementation creates and return a coordinator, having added the store for the application to it. This property is optional since there are legitimate error conditions that could cause the creation of the store to fail.
        // Create the coordinator and store
        var coordinator: NSPersistentStoreCoordinator? = NSPersistentStoreCoordinator(managedObjectModel: self.managedObjectModelUserData)
        
        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("bookmarks.sqlite")
    ...
    lazy var managedObjectContextUserData: NSManagedObjectContext? = {
        // Returns the managed object context for the application (which is already bound to the persistent store coordinator for the application.) This property is optional since there are legitimate error conditions that could cause the creation of the context to fail.
        let coordinator = self.persistentStoreCoordinatorUserData
        if coordinator == nil {
            return nil
        }
        var managedObjectContext = NSManagedObjectContext()
